...
test("fixed virtual canvas scales uniformly without changing world visibility", function()
    ...
    equal(wide.scale, 1); equal(wide.width, 1280); equal(wide.height, 800)
    equal(wide.x, 160); equal(wide.y, 0)
    local wide_world_x, wide_world_y, wide_world_width, wide_world_height = layout.world_viewport(wide)
    equal(wide_world_x, 160); equal(wide_world_y, 160)
    equal(wide_world_width, 960); equal(wide_world_height, 640)
    local tall = layout.physical_transform(1280, 1000)
    equal(tall.scale, 1); equal(tall.width, 1280); equal(tall.height, 800)
    equal(tall.x, 0); equal(tall.y, 100)
    local tall_world_x, tall_world_y, tall_world_width, tall_world_height = layout.world_viewport(tall)
    equal(tall_world_x, 0); equal(tall_world_y, 260)
    equal(tall_world_width, 960); equal(tall_world_height, 640)
    local sixteen_nine = layout.physical_transform(1920, 1080)
    equal(sixteen_nine.scale, 1.35); equal(sixteen_nine.width, 1728); equal(sixteen_nine.height, 1080)
    equal(sixteen_nine.x, 96); equal(sixteen_nine.y, 0)
    local client_x, client_y, client_width, client_height = layout.client_viewport(sixteen_nine)
    equal(client_x, 96); equal(client_y, 0); equal(client_width, 1728); equal(client_height, 1080)
    local sidebar_right = layout.virtual_to_physical(sixteen_nine,
        layout.SIDEBAR.x + layout.SIDEBAR.width, layout.SIDEBAR.y)
    equal(sidebar_right, 1824)
    ...
    local roundtrip_x, roundtrip_y = layout.virtual_to_physical(ultrawide, virtual_x, virtual_y)
    equal(roundtrip_x, 1064); equal(roundtrip_y, 648)
    ...
    assert(layout.physical_to_virtual(ultrawide, 100, 540) == nil,
        "pillarbox input must be rejected")
    for _, transform in ipairs({ native, wide, tall, large }) do
        local world_x, world_y = layout.virtual_to_physical(transform, 480, 480)
        local sidebar_x, sidebar_y = layout.virtual_to_physical(transform, 1000, 400)
        local bottom_x, bottom_y = layout.virtual_to_physical(transform, 480, 80)
        local vx, vy = layout.physical_to_virtual(transform, world_x, world_y)
        assert(layout.is_world_point(vx, vy))
        vx, vy = layout.physical_to_virtual(transform, sidebar_x, sidebar_y)
        assert(layout.is_sidebar_point(vx, vy))
...
test("physical UI dispatch rejects bars and is stable across resolutions", function()
    ...
    for index, size in ipairs(sizes) do
        ...
        assert(equipment_api.equip(equipment, inventory, item_id, "main_hand"))
        ...
        assert(result.success); equal(result.item_id, item_id)
