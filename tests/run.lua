...
test("fixed virtual canvas scales uniformly without changing world visibility", function()
    equal(layout.VIRTUAL_WIDTH, 1280); equal(layout.VIRTUAL_HEIGHT, 720)
    equal(layout.WORLD.x, 0); equal(layout.WORLD.y, 8)
    equal(layout.WORLD.width, 960); equal(layout.WORLD.height, 704)
    equal(layout.SIDEBAR.x, 960); equal(layout.SIDEBAR.width, 320)
    ...
    equal(visible_width, 15); equal(visible_height, 11)
    local center_x, center_y = camera_api.project(camera, 0, 0, 32)
    equal(center_x, 480); equal(center_y, 360)

    local native = layout.physical_transform(1280, 720)
    equal(native.scale, 1); equal(native.x, 0); equal(native.y, 0)
    local large = layout.physical_transform(2560, 1440)
    equal(large.scale, 2); equal(large.x, 0); equal(large.y, 0)
    local small = layout.physical_transform(640, 360)
...
test("engine test map loads its stable herb and key placements", function()
    ...
    local fixed_viewport = viewport_api.new(viewer.position.x, viewer.position.y, 15, 11, 2)
    ...
    equal(fixed_counts.total, 278); equal(fixed_counts.ground, 230)
    equal(fixed_counts.world_objects, 25); equal(fixed_counts.items, 4)
    equal(fixed_counts.actors, 3); equal(fixed_counts.roofs, 16)
