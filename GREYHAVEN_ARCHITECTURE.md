...
10. **UI input and selection** — `render/layout.lua` converts and classifies physical pointer coordinates, while `ui/client_layout.lua` and `ui/input_dispatch.lua` resolve fixed virtual rectangles into semantic, read-only Equipment/Inventory targets. `ui/item_selection.lua` owns one transient identity-based selection; a first click selects and a confirmed second click reaches `ui/equipment_controller.lua`, which translates it into the existing public equip/unequip transfer.
