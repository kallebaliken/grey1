...
10. **UI input dispatch** — `render/layout.lua` converts and classifies physical pointer coordinates, while `ui/client_layout.lua` and `ui/input_dispatch.lua` resolve fixed virtual rectangles into semantic, read-only Equipment/Inventory targets and click intents. `ui/equipment_controller.lua` separately translates allowed clicks into public Equipment transfers: Equipment clicks unequip, while Inventory clicks resolve ItemDefinition compatibility in canonical slot order and equip into the first empty compatible slot without swapping.
