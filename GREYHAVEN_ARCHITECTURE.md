...
- Item drag state is also transient and never owns an item. Six virtual pixels promote an occupied-slot press candidate into a captured drag; release is the only mutation boundary. Inventory-to-Equipment respects the exact authored compatible slot, Equipment-to-Inventory lets the Container choose insertion, Inventory-to-Inventory delegates to explicit Container ordering semantics, and world/Equipment-to-Equipment directions still cancel without transfer.
- Visible Inventory-to-Inventory drops delegate to the compact ordered Container: empty positions append with deterministic shifting, occupied entries swap, and compatible stacks merge toward the destination ID with partial source identity preserved. Only indices 1–16 are interactive; overflow order and occupancy remain authoritative, and Save Format v5 already persists the resulting ordered snapshot.
