...
The complete Greyhaven client is one fixed 1280×800 virtual surface. The physical window never lays out Greyhaven UI; it only uniformly scales and centres that surface. Consequently physical resolution is not UI layout resolution, and a larger monitor never grants more world visibility.

Defold normally adjusts GUI node geometry for the physical window before the render script draws it. That automatic adjustment was a second transform layered on top of Greyhaven's custom client viewport, causing the GUI to diverge from the correctly boxed world and pushing the sidebar out of wide windows. `main/world.gui` therefore uses `ADJUST_REFERENCE_DISABLED`: every GUI node stays in authored 1280×800 coordinates, and the render script's shared client viewport is the sole virtual-to-physical transform for both GUI and world. No node is repositioned during resize.

The custom pipeline targets Defold 1.13.1. Buffer-clear keys and supported blend/depth state values come from `graphics` (`BUFFER_TYPE_COLOR0_BIT`, `BUFFER_TYPE_DEPTH_BIT`, `BUFFER_TYPE_STENCIL_BIT`, blend/depth states and factors); `render` owns the operations themselves. This version exposes neither a supported scissor render state nor `render.set_scissor`. World clipping therefore uses the rasterization viewport itself: the renderer derives the physical world rectangle from the same client scale and offsets, selects that 960×640-scaled rasterization viewport with a matching world-only orthographic projection, draws world sprites, then restores the shared complete-client viewport and 1280×800 projection before GUI drawing. Required constants are asserted during initialization so an API mismatch fails descriptively.
...
GUI nodes use the same fixed 1280×800 virtual coordinates and are grouped beneath client, world-frame, sidebar, bottom, dialogue, and debug roots. The sidebar contains the read-only Equipment and Inventory panels plus map, status, and utility scaffolding. The bottom panel provides nonfunctional Log/Dialogue/Combat/Journal tabs, current notice text, and debug-only controls. Dialogue remains a GUI overlay in the lower world view and does not resize it. Keyboard input is unaffected. Future mouse input must first apply `layout.physical_to_virtual()` (which rejects coordinates outside the client), then use `is_world_point`, `is_sidebar_point`, or `is_bottom_panel_point` before any world-tile mapping.
