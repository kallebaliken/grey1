...
3. **Physical window:** arbitrary size. `render/layout.lua` computes one uniform `min(physical_width / 1280, physical_height / 720)` scale and centred letterbox/pillarbox offsets. The custom render script projects the complete virtual canvas through that physical viewport and applies a physical scissor for the world rectangle.

The custom pipeline targets Defold 1.13.1. Buffer-clear keys and supported blend/depth state values come from `graphics` (`BUFFER_TYPE_COLOR0_BIT`, `BUFFER_TYPE_DEPTH_BIT`, `BUFFER_TYPE_STENCIL_BIT`, blend/depth states and factors); `render` owns the operations themselves. Scissoring is not an `enable_state` capability in this version: `render.set_scissor(x, y, width, height)` establishes the clip and the zero-argument `render.set_scissor()` resets it before GUI drawing. Required constants are asserted during render-script initialization so an API mismatch fails with a specific message instead of a nil table-key error.
