...
The custom pipeline targets Defold 1.13.1. Buffer-clear keys and render-state values come from `graphics` (`BUFFER_TYPE_COLOR0_BIT`, `BUFFER_TYPE_DEPTH_BIT`, `BUFFER_TYPE_STENCIL_BIT`, blend/depth/scissor states and factors); `render` owns the operations themselves. Required constants are asserted during render-script initialization so an API mismatch fails with a specific message instead of a nil table-key error.
