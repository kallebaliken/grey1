...
`combat/armor.lua` then reads all target equipment through its public snapshot API, sums validated `armor.defense` metadata, and applies `final damage = max(1, raw damage - total defense)`. Missing equipment and equipped non-armor contribute zero. Results/events retain raw damage, total armor, final damage, and deterministic source identities; only final damage reaches `combat.registry.apply_damage`. Armor never mutates health, degrades, rolls randomly, or changes attack cooldown.

Defense skills/formulas, shielding, blocking, hit locations, accuracy, misses, critical hits, resistances, durability, weapon-speed stats, ranged attacks, ammo, projectiles, spells, mana, healing, conditions, AI attacks, target-selection AI, chasing, XP, loot, corpses, animations, and combat GUI remain deferred.
