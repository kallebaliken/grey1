...
The dispatcher only identifies the clicked slot; the controller interprets it; ItemDefinition declares compatibility; Equipment and Inventory/Container perform authoritative mutations; and the Inventory and Equipment panels independently reflect new snapshots. The GUI never transfers or rearranges an item directly. Hover only changes the authored slot node's color. F1 reports physical and virtual pointer coordinates, region, hover target, last clicked target, and whether dialogue blocked that intent. Stack splitting, right click, tooltips, and world mouse targeting remain deferred.
...
Inventory-slot drops are interpreted by the interaction controller and delegated to `Inventory.drop_slot`, never rearranged in GUI state. Empty compact-sequence positions mean append, occupied incompatible entries swap, and compatible same-type/state stacks merge toward the destination identity. Partial merges preserve the source ID and remainder; a full destination swaps. Selection follows the dragged ID for moves/swaps/partial merges and explicitly follows the surviving destination ID after a full merge. Equipment-to-Inventory still uses normal authoritative insertion rather than exact destination placement.
