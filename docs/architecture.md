...
Perception is a stateless pure-Lua query over current Actor positions and explicit logical sight blockers. Creature composition associates optional sight range outside Actor state; Manhattan range, same-Z checks, living/active policy, and deterministic centre-line LOS produce stable Actor-ID-ordered awareness snapshots. Optional faction enrichment does not filter results, and perception never invokes movement, pathfinding, attacks, or behavior.
...
World presentation uses atlas-backed Defold sprite Game Objects created by one collection factory. The adapter retains instances by stable render-piece identity, updates their camera-projected position and atlas animation, and removes only pieces that leave the visible command set. Semantic layers and logical Z remain pure-Lua sorting inputs; the adapter maps the final deterministic order into a bounded Defold depth interval. GUI is reserved for status, F1 diagnostics, and dialogue.
