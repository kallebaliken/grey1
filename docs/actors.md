...
The player is an Actor composed at the runtime root with inventory and equipment. NPCs and monsters use the same Actor representation; the engine test includes one inert NPC. Pathfinding is a separate query subsystem, while dialogue, schedules, AI, combat, health, and stats remain absent.
