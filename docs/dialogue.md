...
- Future Conditions may decide which choices are available.
- Future Actions may perform quests, items, or reputation changes.

Only the first four exist now, and faction data does not alter dialogue. Active sessions are temporary runtime state and are never saved; loading starts with no conversation.
