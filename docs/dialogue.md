...
- Actions explicitly mutate boolean flags or QuestState through validated public APIs.

Conditions are read-only and faction data does not alter dialogue. The villager's authored choices explicitly start, advance, and complete `rat_problem`; there is no runtime quest branch or automatic progression. Choice actions run in authored order before the node/close transition and dialogue lifecycle events; node viewing never runs actions. Active sessions are temporary runtime state and are never saved, while restored QuestState immediately determines available choices.
