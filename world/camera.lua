...
function M.new(view_width, view_height, zoom, origin_x, origin_y)
    zoom = zoom or M.DEFAULT_ZOOM
    assert(valid_zoom(zoom), "camera zoom must be an integer from 1 to 3")
    return { width = view_width, height = view_height, origin_x = origin_x or 0, origin_y = origin_y or 0,
        x = 0, y = 0, zoom = zoom }
end
...
function M.project(camera, world_x, world_y, tile_size)
    ...
    return camera.origin_x + camera.width / 2 + (world_x - camera.x) * display_tile_size,
        camera.origin_y + camera.height / 2 + (world_y - camera.y) * display_tile_size
end
