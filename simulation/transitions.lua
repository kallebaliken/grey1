...
function M.resolve(world, actor, instance, transition, events)
    ...
    else destination = position.offset(actor.tile_position, transition.dx or 0, transition.dy or 0, transition.dz or 0) end
    if not world:is_walkable(destination.x, destination.y, destination.z, actor.id) then return false, "destination_blocked" end
    local old_z = actor.tile_position.z
    movement.teleport(world, actor, destination)
    if events then events.emit("z_level_changed", { actor_id = actor.id, object_id = instance.id,
